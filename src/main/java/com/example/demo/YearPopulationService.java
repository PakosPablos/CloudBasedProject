...
public class YearPopulationService {
    ...
    // Data query URL – you can keep "Flat" and add format=csvfilewithlabels
    ...
    public List<YearPopulation> getPopulationForYear(int requestedYear) {
       ...
       // Parse the DF_B1100 CSV and build one YearPopulation per year
// Parse the DF_B1100 CSV and build one YearPopulation per year
...
private Map<Integer, YearPopulation> parseCsvToYearPopulation(String csv) {
    ...
    for (int i = 0; i < cols.length; i++) {
        ...
        if (upper.equals("TIME_PERIOD")) {
            ...
            valueIndex = i;
        } else if (col.equals("Specification")) { // EXACT label column ("Total males", etc.)
            specIndexLabel = i;
        }
    }
    ...
    for (int li = 1; li < lines.length; li++) {
        ...
        String yearStr   = cells[yearIndex].trim();      // e.g. "1821-01-01"
        String valueStr  = cells[valueIndex].trim();     // may be empty or a number
        String specLabel = cells[specIndexLabel].trim().toUpperCase(); // e.g. "LUXEMBOURGISH MALES"

        if (yearStr.isEmpty() || specLabel.isEmpty() || valueStr.isEmpty()) {
            continue;
        }

        // Year is a DATE like 1821-01-01 → take first 4 chars
        ...
        // --- 4) Nationality & sex from your row names ---
        ...
        // "Foreign males"
        // "Total females"
        // "Luxembourgish females"
        // "Femmes étrangères" (foreign females)

        boolean isLux =
                specLabel.contains("LUXEMBOURGISH")
             || specLabel.contains("LUXEMBOURGEOIS");   // just in case French

        boolean isForeign =
                specLabel.contains("FOREIGN")
             || specLabel.contains("ETRANGER");         // "étrangères" → "ETRANGERES" in uppercase

        boolean isMale =
                specLabel.contains("MALE")
             || specLabel.contains("MAL")      // covers "MALES"
             || specLabel.contains("HOMME")
             || specLabel.contains("HOMMES");

        boolean isFemale =
                specLabel.contains("FEMALE")
             || specLabel.contains("FEMME")
             || specLabel.contains("FEMMES");

        // Skip rows that are "Total population" or "Population per km²"
        if (specLabel.contains("TOTAL POPULATION")
                || specLabel.contains("POPULATION PER KM")) {
            continue;
        }

        // Also skip "Total males"/"Total females" (we compute totals from lux+foreign)
        if (specLabel.startsWith("TOTAL MALES") || specLabel.startsWith("TOTAL FEMALES")) {
            continue;
        }

        // If we still don't know nationality or sex, skip
        if (!isLux && !isForeign) continue;
        if (!isMale && !isFemale) continue;

        if (isLux) {
            addToBreakdown(lux, isMale, isFemale, value);
        } else if (isForeign) {
            addToBreakdown(fore, isMale, isFemale, value);
        }

        // Update total population (lux + foreign)
        yp.setTotalPopulation(lux.getTotal() + fore.getTotal());
    }
